...
export async function GET(): Promise<NextResponse> {
  ...
  const p = await getUserPersona(identity.prisonId, identity.name);
  return NextResponse.json({ name: identity.name, persona: p?.persona ?? "" });
}
...
export async function PATCH(req: Request): Promise<NextResponse> {
  ...
  await setUserPersona(identity.prisonId, identity.name, body.persona.trim());
